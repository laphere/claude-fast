...
- **展开会话列表** → 点击项目行展开/收起其 Claude Code 会话列表（标题 + 时间 + 摘要，悬停可看完整标题）；**右键会话行**打开操作菜单：✎ 重命名（写入 jsonl 的 custom-title，Claude Code 官方 /rename 同机制，改名后官方 CLI 同样生效）、🗑 删除（二次确认，删除后移入回收站）、▶ 继续对话、📌 置顶
...
- **▶ 继续对话** → 右键会话行选「继续对话」（或右侧「继续对话」按钮），新开终端窗口在项目目录执行 `claude --resume <会话id>` 继续对话（窗口保持不关）
