...
cd src-tauri && cargo test   # 后端单元测试（171 个：路径解析/配置/扫描/根目录定位/会话管理/mangle/sh_quote/回收站/台账/供应商/模型拉取/版本升级/对话翻译层）
