...
- **app 内嵌终端（node-pty + xterm.js，2026-09-21 自 Tauri `embedded-terminal` 分支移植，该分支已退休、留本机 tag `embedded-terminal-final`）**：与页面对话共存于同一条 tab 栏（tab 模型为判别联合 `kind: "chat" | "term"`，状态收在 tab 对象上）。真 claude CLI 跑在 PTY 里，`TerminalPane.tsx` + `src/lib/{pty,bold-bright,term-unicode,term-title,ime-anchor}.ts` 与 `styles.css` 终端样式块**逐字搬运**（含注释里的实测数据，别改）。要点：
...
- **app 内直接对话（官方 Agent SDK）**：`electron/backend/chat.ts` 用 `@anthropic-ai/claude-agent-sdk`（运行时动态 `import()`）托管 CLI 子进程，事件翻译成 `ChatEvent` 经 IPC 推给 `ChatView` 流式渲染。要点：
  ...
  - **模型热切**（`supportedModels()` + `setModel()`，2026-09-24 探针+落地，口径见能力面文档 §6.13）：composer 中部那个模型名从只读 span 变成 `ModelPicker`（结构照 ModePicker，复用 `.mode-picker` class，另加 `.model-picker` 覆盖它的 `flex-shrink:0`——模型名长度不可控，不缩会把同行上下文占用挤出容器）。四条不能改的：① **条目是供应商映射后的槽位**——第三方供应商下 opus/fable/sonnet/haiku 各自解析到不同模型变体（`[1m]`/`[1M]` 大小写即两回事），每行亮出 `resolvedModel` 且**不归一化大小写**；② **选中态按「用户点过的槽位」判，退路才是「第一个解析值匹配的行」**——实测 GLM 下 opus/fable/sonnet **三个槽位都解析到 `glm-5.3[1M]`**，只按 resolvedModel 判会三行同时高亮、且点另一个槽位被「同一个模型就不发」的早退挡成静默无操作（2026-09-24 code review 发现）；③ 切换成功后 CLI **立即**补发一帧 init（不等下一轮）→ 经既有 `session_ready` 事件送新模型名——**不乐观更新、不加新事件**；④ 进程没起时后端返回 null、面板显示「发送首条消息后可选」（**v1 不做 spawn 前选模型**，`Options.model` 那条路未探）。effort 选择器被砍：SDK 宿主上 effort **零回显**（applyFlagSettings 成功但没有任何帧带它），盲控制器 + resume 后状态丢失，等有回显通道再做。
