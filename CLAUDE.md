...
>
> **引用提示**：本文多处拿 **`v2.0.0`**（Tauri 2 + Rust、含 app 内 GUI 对话的那条线）当行为对照——该分支**已于 2026-09-21 删除**（本地与远端都不在了，只剩本机 tag `v2.0.0-final`），那些引用只作历史标注、事后查不到代码。同日 Tauri 的另一条线 **`v1.0.0`（查看器/启动器 + 内嵌终端）也删了**：内容先 ff 进 `embedded-terminal`（两者本就同内容），再删 `v1.0.0` 的本地与远端分支（留本机 tag `v1.0.0-final`）。**重组后只剩三支**：本分支 `claude-fast-electron`（**当前主线**）、`embedded-terminal`（Tauri 线，与 `main` 同一提交）；一条线一个分支，不再有版本号命名的分支。
>
